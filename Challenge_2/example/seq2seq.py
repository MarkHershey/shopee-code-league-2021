...
class Decoder(nn.Module):
    ...
    def forward(self, _input, hidden, cell):
...
def evaluate(model, iterator, criterion):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(iterator):
            ...
            # logger.info(f"{type(src)}")
            # logger.info(f"{str(src)}")
...
# def inference(addr_input, src_field, trg_field, model, device, max_len=50):

#     model.eval()

#     if isinstance(addr_input, str):
#         tokens = list(addr_input.split())
#     else:
#         tokens = [token.lower() for token in addr_input]
